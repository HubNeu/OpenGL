...
glSphere::glSphere(float R, char *TextureFile)
{
	...
	while (phi <= 90)
	{
		...
		while (alpha <= 360.0)
		{
			...
			AddVertex(R*cos(alpha*PI / 180)*cos(phi*PI / 180), sin(phi*PI / 180)*R, R*sin(alpha*PI / 180) *cos(phi*PI / 180), u, v);
			AddVertex(R*cos(alpha*PI / 180) *cos((phi + dphi)*PI / 180), sin((phi + dphi)*PI / 180)*R, R*sin(alpha*PI / 180)*cos((phi + dphi)*PI / 180), u, v + dv);
			alpha += dalpha;
			u+=du;
		}
		phi += dphi;
