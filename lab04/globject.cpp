...
void glObject::MakeEgg(GLfloat a,GLfloat b,GLfloat slices, GLfloat stacks)
{
  ...
  {
	  //if (trigger) { SetColor(0.0f, 0.5f, 0.0f); trigger = false; }//dziala nie do konca jak planowane bylo
	  //else { SetColor(0.0f, 1.0f, 0.0f); trigger = true; }
	  ...
	  {
		  if (tick % 4 <= 1) { SetColor(0.0f, 1.0f, 0.0f); }
		  else { SetColor(0.0f, 0.5f, 0.0f); }
		  
		  phi2 = phi1 + stack_delta; // oblicz phi2 wzgledem phi1

		  v1[0] = x;
		  v1[1] = r1*cos(phi1*3.14 / 180);
		  v1[2] = r1*sin(phi1*3.14 / 180);
		  v2[0] = x;
		  v2[1] = r1*cos(phi2*3.14 / 180);
		  v2[2] = r1*sin(phi2*3.14 / 180);
		  v3[0] = x + slice_delta;
		  v3[1] = r2*cos(phi2*3.14 / 180);
		  v3[2] = r2*sin(phi2*3.14 / 180);
		  v4[0] = x + slice_delta;
		  v4[1] = r2*cos(phi1*3.14 / 180);
		  v4[2] = r2*sin(phi1*3.14 / 180);

		  // pierwszy trojkat
		  CalcNormal(v1, v2, v3, N);
		  Normalize(N);
		  AddVertex(v1[0], v1[1], v1[2], N[0], N[1], N[2]);
		  AddVertex(v2[0], v2[1], v2[2], N[0], N[1], N[2]);
		  AddVertex(v3[0], v3[1], v3[2], N[0], N[1], N[2]);
		  ...
		  AddVertex(v1[0], v1[1], v1[2], N[0], N[1], N[2]);
		  AddVertex(v3[0], v3[1], v3[2], N[0], N[1], N[2]);
		  AddVertex(v4[0], v4[1], v4[2], N[0], N[1], N[2]);
...
void glObject::AddVertex(float x, float y, float z, float nx, float ny, float nz)
{
	lCoords[lVAO - 1] += 3;
	Coords = (float *)realloc(Coords, lCoords[lVAO - 1] * sizeof(float));
	if (Coords == NULL) ThrowException("glObject:: Blad realokacji pamieci");
